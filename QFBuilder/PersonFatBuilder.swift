...
class PersonFatBuilder: PersonBuilder {
    ...
    override func drawBody() {
        ...
        path.move(to:CGPoint.init(x:148, y:125))//移动到圆底部
        path.addLine(to:CGPoint.init(x:148, y:175))//画腰
        
        path.move(to:CGPoint.init(x:152, y:125))//移动到圆底部
        path.addLine(to:CGPoint.init(x:152, y:175))//画腰
